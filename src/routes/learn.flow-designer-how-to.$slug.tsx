...
import { getFlowHowToGuide, type FlowHowToGuide } from "@/lib/content/flow-howto";

const BASE = "https://www.sparkcoder.online/learn/flow-designer-how-to";

export const Route = createFileRoute("/learn/flow-designer-how-to/$slug")({
  loader: ({ params }) => {
    const guide = getFlowHowToGuide(params.slug);
...
function GuideNotFound() {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-4 p-8 text-center">
      ...
      <Link to="/learn/flow-designer-how-to" className="text-accent hover:underline text-sm">
        ← All Flow Designer how-tos
      </Link>
    </div>
  );
}

function FlowHowToDetail() {
  const { progress } = useProgress();
  const { guide } = Route.useLoaderData() as { guide: FlowHowToGuide };
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-3xl w-full mx-auto p-5 sm:p-8 space-y-8 pb-24">
        <nav aria-label="Breadcrumb" className="text-[11px] text-muted-foreground">
          <Link to="/learn/flow-designer-how-to" className="hover:text-accent">
